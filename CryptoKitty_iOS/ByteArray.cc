...
void ByteArray::copy(unsigned offset, const ByteArray& other,
    ...
    if ((otherOffset + transfer) > other.getLength()) {
...
void ByteArray::copy(unsigned offset, const uint8_t *other,
    ...
    copy(offset, ByteArray(other+otherOffset, length), length);
...
void ByteArray::truncate(unsigned size) {
...
coder::ByteArray operator^ (const coder::ByteArray& lhs, const coder::ByteArray& rhs) {
    ...
    coder::ByteArray result(lhs.getLength());
